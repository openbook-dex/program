...
fn test_new_order() {
    ...
    {
        let market = Market::load(&accounts.market, &dex_program_id, false).unwrap();
        assert_eq!(identity(market.referrer_rebates_accrued), 0);
        assert_eq!(identity(market.pc_fees_accrued), 0);
        assert_eq!(
    ...
    }
    {
        ...
        assert_eq!(identity(open_orders_seller.native_pc_free), 400_000);
        assert_eq!(identity(open_orders_seller.native_pc_total), 400_000);
    }
